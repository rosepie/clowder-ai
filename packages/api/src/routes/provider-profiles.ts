...
import {
  ...
  resolveRuntimeAcpModelProfileById,
...
import { buildACPModelProfileOverridePayload } from '../domains/cats/services/agents/providers/acp-model-profile-override.js';
...
const protocolEnum = z.enum(['anthropic', 'openai', 'google', 'acp']);
const authTypeEnum = z.enum(['oauth', 'api_key', 'none']);
const modeEnum = z.enum(['subscription', 'api_key', 'none']);
const kindEnum = z.enum(['api_key', 'acp']);
const acpModelAccessModeEnum = z.enum(['self_managed', 'clowder_default_profile']);
const acpModelProviderEnum = z.enum(['openai_compatible', 'bigmodel', 'minimax', 'echo']);
...
const createAcpModelProfileBodySchema = z
  .object({
    projectPath: z.string().optional(),
    name: z.string().trim().min(1).optional(),
    displayName: z.string().trim().min(1).optional(),
...
const updateAcpModelProfileBodySchema = z.object({
  projectPath: z.string().optional(),
  name: z.string().trim().min(1).optional(),
  displayName: z.string().trim().min(1).optional(),
  ...
  app.post('/api/provider-profiles', async (request, reply) => {
    ...
    try {
      const profile = await createProviderProfile(projectRoot, {
        ...(body.kind != null ? { kind: body.kind as ProviderProfileKind } : {}),
        ...
        ...(body.args != null ? { args: body.args } : {}),
        ...(body.cwd != null ? { cwd: body.cwd } : {}),
        ...(body.modelAccessMode != null ? { modelAccessMode: body.modelAccessMode } : {}),
        ...(body.defaultModelProfileRef != null ? { defaultModelProfileRef: body.defaultModelProfileRef } : {}),
